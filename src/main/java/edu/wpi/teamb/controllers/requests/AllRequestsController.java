...
import edu.wpi.teamb.entities.requests.AllRequests;
...
public class AllRequestsController {
    ...
    private void loadRequestsIntoContainer() {
        ...
        ArrayList<Request> listOfRequests = allRequests.getAllRequests();
        for (int i = 0; i < listOfRequests.size(); i++) {
            ...
            switch (listOfRequests.get(i).getRequestType()) {
                case "Meal":
                    FullMealRequest fullMealRequest = allRequests.getMealRequest(listOfRequests.get(i).getId());
                    requestInfoCardController.sendRequest(fullMealRequest);
                    break;
                case "Conference":
                    FullConferenceRequest fullConferenceRequest = allRequests.getConfenferenceRequest(listOfRequests.get(i).getId());
                    requestInfoCardController.sendRequest(fullConferenceRequest);
                    break;
                case "Flower":
                    FullFlowerRequest fullFlowerRequest = allRequests.getFlowerRequest(listOfRequests.get(i).getId());
                    requestInfoCardController.sendRequest(fullFlowerRequest);
                    break;
                case "Office":
                    FullOfficeRequest fullOfficeRequest = allRequests.getOfficeRequest(listOfRequests.get(i).getId());
